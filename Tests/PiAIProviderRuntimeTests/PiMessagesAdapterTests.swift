...
struct PiMessagesAdapterTests {
  ...
  func encodesContextAndNormalizesTextReasoningToolUsageAndTerminal() async throws {
    ...
    let messages = try #require(context.array("messages"))
    #expect(messages.count == 4)
    let system = try #require(messages.first?.objectValue)
    #expect(system.string("role") == "system")
    #expect(system.string("content") == "Be precise")
    #expect(system.array("toolsAdded")?.count == 1)
    #expect(context["systemPrompt"] == nil)
    #expect(context["tools"] == nil)
